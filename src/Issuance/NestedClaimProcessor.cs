...
internal class NestedClaimProcessor
{
        ...
        List<string> disclosures)
    {
        ...
        // Determine which properties should be selectively disclosable
        var selectivePropertyNames = nestedPaths
            .Select(p => p.PathComponents[1]) // First nested level (e.g., "city" from "address.city")
            .ToHashSet();

        foreach (var (propertyName, propertyValue) in allProperties)
        {
            if (selectivePropertyNames.Contains(propertyName))
            {
                // This property should be selectively disclosable
                // Generate disclosure for this nested property
                var disclosure = _disclosureGenerator.GenerateDisclosure(propertyName, propertyValue);
                ...
                // Don't add this property to the result object - it's now hidden
            }
            else
            {
                // Non-selective property - include as-is
                // Deserialize to native type for clean JSON serialization
