...
internal static partial class FieldOperations
{
    ...
    internal static void fe_cmov(ref FieldElement f, ref FieldElement g, int b)
    {
        ...
        int x0 = mask & (f.x0 ^ g.x0); f.x0 ^= x0;
        int x1 = mask & (f.x1 ^ g.x1); f.x1 ^= x1;
        int x2 = mask & (f.x2 ^ g.x2); f.x2 ^= x2;
        int x3 = mask & (f.x3 ^ g.x3); f.x3 ^= x3;
        int x4 = mask & (f.x4 ^ g.x4); f.x4 ^= x4;
        int x5 = mask & (f.x5 ^ g.x5); f.x5 ^= x5;
        int x6 = mask & (f.x6 ^ g.x6); f.x6 ^= x6;
        int x7 = mask & (f.x7 ^ g.x7); f.x7 ^= x7;
        int x8 = mask & (f.x8 ^ g.x8); f.x8 ^= x8;
        int x9 = mask & (f.x9 ^ g.x9); f.x9 ^= x9;
    }
