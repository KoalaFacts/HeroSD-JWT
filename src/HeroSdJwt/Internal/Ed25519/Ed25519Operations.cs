...
internal static class Ed25519Operations
{
    ...
    /// </summary>
    internal static void CryptoSignKeypair(
        ...
        byte[] seed, int seedoffset)
    {
        ...
        GroupElementP3 A;
        GroupOperations.GeScalarmultBase(out A, hash, 0);
        GroupOperations.GeP3ToBytes(pk, pkoffset, ref A);
    ...
    ///   sk[skoffset..skoffset+63]: 64-byte expanded private key
    /// </summary>
    internal static void CryptoSign(
        ...
        byte[] sk, int skoffset)
    {
        ...
        GroupOperations.GeP3ToBytes(publicKey, 0, ref A);
        ...
        GroupOperations.GeP3ToBytes(sig, sigoffset, ref R);
        ...
        ScalarOperations.ScMuladd(s_result, hram, hash, r);
    ...
    /// </summary>
    internal static bool CryptoSignVerify(
        byte[] sig, int sigoffset,
        byte[] m, int moffset, int mlen,
        byte[] pk, int pkoffset)
    {
        ...
        GroupElementP3 A;
        if (!GroupOperations.GeFromBytesNegateVartime(out A, pk, pkoffset))
            return false;

        // Negate A for the verification equation [s]B - [k]A = R
        // We compute this as [s]B + [k](-A) using Shamir's trick
        FieldOperations.FeNeg(out A.X, ref A.X);
        FieldOperations.FeNeg(out A.T, ref A.T);
        ...
        GroupElementP2 R;
        GroupOperations.GeDoubleScalarmultVartime(out R, hram, ref A, s);

        // Encode R and compare with sig[0..31]
        byte[] rcheck = new byte[32];
        GroupOperations.GeToBytes(rcheck, 0, ref R);
    ...
    /// </summary>
    private static bool ScIsValid(byte[] s)
    {
