...
/// <summary>
/// Unit tests for SignatureValidator with HS512 algorithm.
/// Tests HMAC-SHA512 signature verification.
/// </summary>
public class SignatureValidatorHS512Tests
{
    private readonly SignatureValidator _validator = new();
    private readonly JwtSigner _signer = new();

    [Fact]
    public void VerifyJwtSignature_ValidHS512Signature_ReturnsTrue()
    {
        // Arrange
        ...
        // Assert
        Assert.True(result, "Valid HS512 signature should verify successfully");
    }
    ...
    public void VerifyJwtSignature_TamperedHeader_ReturnsFalse()
    {
        ...
        // Tamper with header
        var parts = jwt.Split('.');
        var tamperedHeader = Base64UrlEncoder.Encode(JsonSerializer.Serialize(new { alg = "HS512", typ = "TAMPERED" }));
    ...
    public void VerifyJwtSignature_WrongKey_ReturnsFalse()
    {
        // Arrange
        var key = RandomNumberGenerator.GetBytes(64);
        var wrongKey = RandomNumberGenerator.GetBytes(64);
        var jwt = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = "user123" }, key, SignatureAlgorithm.HS512);
        ...
        // Assert
        Assert.False(result, "Invalid HS512 signature with wrong key should fail verification");
    }
    ...
    public void VerifyJwtSignature_InvalidSignatureLength_ReturnsFalse()
    {
        ...
        var invalidSignature = Base64UrlEncoder.Encode(new byte[32]); // 32 bytes instead of 64
    ...
    public void VerifyJwtSignature_DifferentPayloadSizes_VerifiesCorrectly()
    {
        ...
        var jwtSmall = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = smallPayload.sub }, key, SignatureAlgorithm.HS512);
        var jwtMedium = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = mediumPayload.sub, ["iss"] = mediumPayload.iss, ["aud"] = mediumPayload.aud }, key, SignatureAlgorithm.HS512);
        var jwtLarge = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = largePayload.sub, ["iss"] = largePayload.iss, ["aud"] = largePayload.aud, ["claims"] = largePayload.claims }, key, SignatureAlgorithm.HS512);
