...
/// <summary>
/// Unit tests for SignatureValidator with ES512 algorithm.
/// Tests ECDSA with P-521 curve and SHA-512 signature verification.
/// </summary>
public class SignatureValidatorES512Tests
{
    private readonly SignatureValidator _validator = new();
    private readonly JwtSigner _signer = new();

    [Fact]
    public void VerifyJwtSignature_ValidES512Signature_ReturnsTrue()
    {
        // Arrange
        ...
        // Assert
        Assert.True(result, "Valid ES512 signature should verify successfully");
    }
    ...
    public void VerifyJwtSignature_TamperedHeader_ReturnsFalse()
    {
        ...
        // Tamper with header
        var parts = jwt.Split('.');
        var tamperedHeader = Base64UrlEncoder.Encode(JsonSerializer.Serialize(new { alg = "ES512", typ = "TAMPERED" }));
    ...
    {
        // Arrange
        using var ecdsa1 = ECDsa.Create(ECCurve.NamedCurves.nistP521);
        using var ecdsa2 = ECDsa.Create(ECCurve.NamedCurves.nistP521);
        ...
        // Assert
        Assert.False(result, "Invalid ES512 signature with wrong key should fail verification");
    }

    [Fact]
    public void VerifyJwtSignature_WrongCurveP256_ThrowsSdJwtException()
    {
        // Arrange - Use P-256 instead of required P-521 for ES512
        using var ecdsa = ECDsa.Create(ECCurve.NamedCurves.nistP256);
        var privateKey = ecdsa.ExportPkcs8PrivateKey();

        // Act & Assert - Signer now validates curve during signing
        var exception = Assert.Throws<ArgumentException>(() =>
            _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = "user123" }, privateKey, SignatureAlgorithm.ES512));
        Assert.Contains("P-521", exception.Message);
    }

    [Fact]
    public void VerifyJwtSignature_WrongCurveP384_ThrowsSdJwtException()
    {
        // Arrange - Use P-384 instead of required P-521 for ES512
    ...
    [Fact]
    public void VerifyJwtSignature_P521Curve_VerifiesSuccessfully()
    {
        // Arrange - Explicitly test P-521 curve validation
        using var ecdsa = ECDsa.Create(ECCurve.NamedCurves.nistP521);
        var privateKey = ecdsa.ExportPkcs8PrivateKey();
        var publicKey = ecdsa.ExportSubjectPublicKeyInfo();
        var jwt = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = "user123" }, privateKey, SignatureAlgorithm.ES512);
        ...
        // Assert
        Assert.True(result, "P-521 curve should verify successfully for ES512");
    }
    ...
    public void VerifyJwtSignature_DifferentPayloadSizes_VerifiesCorrectly()
    {
        ...
        var jwtSmall = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = smallPayload.sub }, privateKey, SignatureAlgorithm.ES512);
        var jwtMedium = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = mediumPayload.sub, ["iss"] = mediumPayload.iss, ["aud"] = mediumPayload.aud }, privateKey, SignatureAlgorithm.ES512);
        var jwtLarge = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = largePayload.sub, ["iss"] = largePayload.iss, ["aud"] = largePayload.aud, ["claims"] = largePayload.claims }, privateKey, SignatureAlgorithm.ES512);
    ...
    public void VerifyJwtSignature_NonDeterministicSignatures_BothVerify()
    {
        ...
        var jwt1 = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = payload.sub }, privateKey, SignatureAlgorithm.ES512);
        var jwt2 = _signer.CreateJwt(new Dictionary<string, object> { ["sub"] = payload.sub }, privateKey, SignatureAlgorithm.ES512);
        ...
        // Assert
        Assert.True(result1, "First ES512 signature should verify successfully");
